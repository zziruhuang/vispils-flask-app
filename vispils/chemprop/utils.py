...
def load_checkpoint(path: str,
    ...
    # Matching training args and current args
    if current_args is not None:
        #    args = current_args
        checkpoint_args_dict=vars(args)
        current_args_dict=vars(current_args)

        if checkpoint_args_dict.keys() != current_args_dict.keys():
            new_args = list(current_args_dict.keys() - checkpoint_args_dict.keys())
            for na in new_args:
                checkpoint_args_dict[na]=current_args_dict[na]

        args=Namespace(**checkpoint_args_dict)
