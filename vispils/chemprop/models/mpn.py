...
class MPNEncoder(nn.Module):
    ...
    def forward(self,mol_graph: BatchMolGraph, features_batch=None) -> torch.Tensor:
        if self.use_input_features and features_batch is not None:
            features_batch = torch.from_numpy(np.stack(features_batch)).float().to(self.device)
            if self.features_only:
                return features_batch
        # Unpack only the first 7 outputs from get_components()
        components = mol_graph.get_components()
        if len(components) >= 7:
            f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope = components[:7]
        else:
            raise ValueError('mol_graph.get_components() did not return enough values')
        # Remove or ignore .cuda() calls to force CPU usage
        ...
        for i, (a_start, a_size) in enumerate(a_scope):
            if a_start is None or a_size is None or a_size == 0:
                continue
...
class MPN(nn.Module):
    ...
    def forward(self, batch: Union[List[str], BatchMolGraph],
        ...
        if not self.graph_input and not isinstance(batch, BatchMolGraph):
