...
CORR_FEATURES, CORR_SCALES = pass_corr_scheme()
#CORR_MAX_LENGTH= 42 # for Tm
CORR_MAX_LENGTH= 66 #for viscosity

# Atom feature sizes
MAX_ATOMIC_NUM = 100 #53
...
CORR_DIM=CORR_MAX_LENGTH*len(CORR_FEATURES.keys())
ATOM_FDIM = sum(len(choices) + 1 for choices in ATOM_FEATURES.values()) + 2 + CORR_DIM
...
def get_bond_fdim(args: Namespace) -> int:
    ...
    BOND_FDIM = 14 # + 360 (rdkit 2d features)
...
def atom_features(atom: Chem.rdchem.Atom, functional_groups: List[int] = None) -> List[
...
"""
Functions Added by Ziru
"""
def atom_features_new(atom: Chem.rdchem.Atom, functional_groups: List[int] = None) -> List[
    Union[bool, int, float]]:
    """
    Builds a feature vector for an atom.
    ...
    features = onek_encoding_unk(atom.GetAtomicNum() - 1, ATOM_FEATURES['atomic_num']) + \
               ...
               [1 if atom.GetIsAromatic() else 0] + \
               [atom.GetMass() * 0.01] + \
               [pauling_electronegativity(atom)] + [first_ionization(atom)] + [electron_affinity(atom)] + \
               [atomic_polarisability(atom)]
...
class MolGraph:
    ...
    def __init__(self, smiles: str, args: Namespace):
        ...
        if len(CORR_FEATURES.keys())>0:
            mol_corr = atomic_correlations(self.smiles, max_length=CORR_MAX_LENGTH,
                                    CORR_FEATURES=CORR_FEATURES, CORR_SCALES=CORR_SCALES)

        # Atom Features
        for i, atom in enumerate(mol.GetAtoms()):
            atomic_feature=atom_features(atom)

            if len(CORR_FEATURES.keys())>0:
                try:
                    atomic_feature.extend(mol_corr[i])
                except:
                    print(f"CorrDes failed for atom {i} of {self.smiles} with length {len(mol_corr)}")
                    atomic_feature.extend([0]*CORR_DIM)

            self.f_atoms.append(atomic_feature)
        ...
        for a1 in range(self.n_atoms):
            for a2 in range(a1 + 1, self.n_atoms):
                ...
                # f_bond.extend(get_rdkit_2d(smiles)) # TODO: add rdkit 2d features to bond matrix
